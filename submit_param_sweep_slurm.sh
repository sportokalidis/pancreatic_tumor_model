#!/bin/bash
#SBATCH --job-name=pancreatic_tumor_sweep
#SBATCH --array=0-9
#SBATCH --ntasks=1
...
# SLURM Job Array Script for Pancreatic Tumor Model Parameter Sweep
# 
# Usage:
#   1. Generate parameter files first:
#      python3 generate_param_sweep.py --sweep-type tumor_growth --num-jobs 10
#   2. Submit job array:
#      sbatch submit_param_sweep_slurm.sh
#
# This script will:
...
# Use bdm run with our parameter file and output directory
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo "Start Time: ${SLURM_JOB_START_TIME}" >> "${OUTPUT_DIR}/job_summary.txt"
    echo "End Time: $(date)" >> "${OUTPUT_DIR}/job_summary.txt"
    echo "Exit Code: ${EXIT_CODE}" >> "${OUTPUT_DIR}/job_summary.txt"
    echo "Node: ${SLURMD_NODENAME}" >> "${OUTPUT_DIR}/job_summary.txt"
